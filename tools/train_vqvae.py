...
from utils.mlem import performMLEM_radon as mlem
from utils.addnoise import addnoise
from utils.metric import SSIM_tensor as ssim
...
def train(args):
    ...
    pbeam = ParallelBeam(dataset_config['im_size'],np.linspace(0, np.pi, dataset_config['im_size'], endpoint=False))
    ...
    for epoch_idx in range(num_epochs):
        ...
        for im in tqdm(data_loader):
            ...
            # if arbitrary dose level simulation


            ims=[]
            for i in range(im.shape[0]):
                noisy = random.choice([True, False])
                if noisy:
                    dose = (torch.randint(1,500,(1,)))*1e5
                    dose = dose.float().to(device)
                    noisy_sino = addnoise(im[i].squeeze(),dose)
                    noisy_im = mlem(noisy_sino,noisy_sino)
                    im1 = noisy_im[None,None,].float().to(device)
            ...
            ######### Optimize Generator ##########
            # L2 Loss
            recon_loss = recon_criterion(output, im_gt)
            ...
            lpips_loss = torch.mean(lpips_model(output, im_gt)) / acc_steps
            ...
            if step_count > disc_step_start:
                ...
                disc_real_pred = discriminator(im_gt)
                disc_fake_loss = disc_criterion(disc_fake_pred,
