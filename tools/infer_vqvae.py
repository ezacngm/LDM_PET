...
def infer(args,i):
    ...
    with torch.no_grad():
        ...
        encoder_grid = make_grid(encoded_output.cpu(), nrow=ngrid,normalize=True)
        decoder_grid = make_grid(decoded_output.cpu(), nrow=ngrid,normalize=True)
        input_grid = make_grid(ims.cpu(), nrow=ngrid,normalize=True)
        ...
        input_grid.save(os.path.join(train_config['task_name'], 'slice{:03d}_input.png'.format(i)))
        # encoder_grid.save(os.path.join(train_config['task_name'], 'slice{:03d}_encoded.png'.format(i)))
        decoder_grid.save(os.path.join(train_config['task_name'], 'slice{:03d}_reconstructed.png'.format(i)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', dest='config_path',
                        default='config/ppet.yaml', type=str)
